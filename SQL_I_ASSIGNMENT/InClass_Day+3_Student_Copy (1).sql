...
select product, count(product)as cnt from bank_inventory_pricing where purchase_cost>ifnull(estimated_sale_price,0) group by product having cnt>1;
...
select count(distinct product) as unique_product from bank_inventory_pricing;
...
select month,sum(purchase_cost) from bank_inventory_pricing group by month having month=1 or month=2;
...
where purchase_cost is not null group by product having sum(quantity)>4;

# Question 15:
# 15) Print products whose average of purchase_cost per product is less than 200
#  from the table Bank_inventory_pricing
select product,avg(purchase_cost)from bank_inventory_pricing group by product having ifnull(avg(purchase_cost),0)<200;
...
select product,avg(estimated_sale_price)as original_estimated_sale_price,
avg(estimated_sale_price)*1.15 as new_estimated_sale_price from bank_inventory_pricing group by product having avg(purchase_cost)>avg(estimated_sale_price);
