...
select t1.account_type,sum(t2.transaction_amount),dayname(t2.transaction_date)from bank_account_details t1
join bank_account_transaction t2 on t1.account_number=t2.account_number where t1.account_type='credit Card' group by dayname(t2.transaction_date)='friday',
dayname(t2.transaction_date)!='friday'
...
select t1.account_number,t1.account_type,t2.transaction_date,sum(t2.transaction_amount),t3.holiday
from bank_account_details t1 left join bank_account_transaction t2 on t1.account_number=t2.account_number
left join bank_holidays t3 on t2.transaction_date=t3.holiday group by t3.holiday
...
select t1.account_number,t1.transaction_date,t2.holiday,t3.event,t3.customer_message
from bank_account_transaction t1 join bank_holidays t2 on t1.transaction_date=t2.holiday
cross join bank_customer_messages t3 where t3.event='adhoc' and month(t1.transaction_date)=3
...
select t1.account_number,t1.account_type,t2.transaction_date,t2.transaction_amount,(t2.transaction_amount * t3.interest_rate)as Accrued_int_cal
from bank_account_details t1 join bank_account_transaction t2 on t1.account_number=t2.account_number
join bank_interest_rate t3 on t1.account_type=t3.account_type
where t1.Account_type = 'recurring deposits' and datediff(curdate(),t2.transaction_date) > 30;
...
select t1.account_number as primary_acnt_num, t1.account_type pri_ac_type, t2.account_number
as secondary_accnt_num, t2.account_type sec_ac_type, 
count(t3.transaction_amount) as num_creditcard_trans
from bank_account_details t1 
inner join bank_account_relationship_details t2 on  t1.Account_Number = t2.Linking_Account_Number
inner join bank_account_transaction t3 on t2.Account_Number = t3.Account_Number
where t1.Account_type = 'savings' and t2.Account_type like '%credit%'
group by t1.Account_Number
having num_creditcard_trans >1;


# Question 6:
# 6) Display the Savings Account number whose corresponding AddonCredit card transactions have occured atleast once. 

select t1.account_number as primary_num, t1.account_type primary_type, t2.account_number 
as secondary_num, t2.account_type secondary_type,
count(t3.transaction_amount) as num_creditcard_transaction
from bank_account_details t1
inner join bank_account_relationship_details t2 on  t1.Account_Number = t2.Linking_Account_Number
inner join bank_account_transaction t3 on t2.Account_Number = t3.Account_Number
where t1.Account_type = 'savings' and t2.Account_type = 'Add-on Credit card'
group by t1.Account_Number
having num_creditcard_transaction >= 1 ;
-- empy rows/no transaction found
...
select customer_id,length(customer_id)from bank_customer
...
select * from bank_customer left join bank_customer_export using(customer_name)
